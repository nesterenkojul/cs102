...
def check_solution(solution: tp.List[tp.List[str]]) -> bool:
    ...
    for i in range(len(solution)):
        ...
        check_row = [0 for a, row_element in enumerate(row[:-1]) if row_element in row[a + 1 :]]
        check_col = [0 for b, col_element in enumerate(col[:-1]) if col_element in col[b + 1 :]]
        check_block = [
            0 for c, block_element in enumerate(block[:-1]) if block_element in block[c + 1 :]
        ]
